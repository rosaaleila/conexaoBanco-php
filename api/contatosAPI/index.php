 ...
 * DELETE       - apagar dados
 * PUT/PATH     - editar dados ja existentes
 * 
 ***************************************************************************************/
...
// endpoint: requisitao para editar um contato, simulando o put
    ...
    if (is_numeric($args['id'])) {
        ...
        switch ($contentType[0]) {
            case 'multipart/form-data':
                ...
                // chama a funcao para buscarmos os dados do contato
                if ($dadosContato = buscarContato($id)) {
                    ...
                    if (is_bool($resposta) && $resposta == true) {
                        return $response
                            ->withStatus(201)
                            ->withHeader('Content-Type', 'application/json')
                            ->write('{"message": "Registro editado com sucesso."}');
                    } elseif (is_array($resposta) && isset($resposta['idErro'])) {
                        $dadosJSON = createJSON($resposta);
                    ...
                    return $response
                        ->withStatus(404)
                        ->withHeader('Content-Type', 'application/json')
                        ->write('{"idErro": 404, "message": "Não existe um registro com o ID informado."}');
                }
                break;
        ...
        return $response
            ->withStatus(404)
            ->withHeader('Content-Type', 'application/json')
            ->write('{"idErro": 404, "message": "Não é possível editar um registro sem um ID válido (númerico)."}');
    }
});
